...
<div class="apparatus-3d" data-setup="faraday-induction"></div>

<div class="apparatus-photo">
<img src="../../assets/apparatus/faraday-induction.png" alt="Apparatus layout diagram for this experiment">
...
<div class="apparatus-3d-caption"><b>3-D apparatus schematic</b> &middot; drag to rotate, scroll to zoom &middot; A virtual coil linked to a bar magnet that can be moved at a controllable speed, plus an independently-adjustable external field source (for studying dB/dt with a stationary coil), with a galvanometer/oscilloscope trace of the induced EMF.</div>
</div>
...
Faraday's law states that the EMF induced in a circuit equals the negative rate of change of magnetic flux linkage through it: ε = −N·dΦ_B/dt, where N is the number of turns and Φ_B = ∫B·dA is the flux through one turn. The flux can change because the field itself varies in time, because the coil's area or orientation changes, or because a magnet moves relative to the coil. The negative sign is Lenz's law: the induced current always flows in the direction that opposes the change producing it, consistent with energy conservation — work must be done against the induced effects to change the flux, and that work is what appears as electrical energy in the circuit.
...
Launch the [simulator](../../../labs/advanced-electricity-magnetism-lab.html){target=_blank} and use the following controls on the **Faraday's Law of Electromagnetic Induction** panel:

| Control | Symbol | Range | Default |
|---|---|---|---|
| Number of turns | N | 10–500  | 100  |
| Magnet approach speed | v | 1–50 cm/s | 10 cm/s |
| Peak field of the magnet | B_max | 5–100 mT | 30 mT |
| Coil cross-sectional area | A | 5–50 cm² | 20 cm² |

## Procedure

1. Move the magnet toward the coil at a fixed speed and record the induced EMF pulse on the virtual oscilloscope; note its sign.
2. Repeat, withdrawing the magnet at the same speed, and note the induced EMF reverses sign.
3. Double the approach speed and verify the peak EMF approximately doubles (steeper dB/dt).
4. Increase the number of turns N and verify the induced EMF scales proportionally.
5. Hold the magnet stationary inside the coil and confirm the induced EMF is exactly zero (no change in flux).
6. Use the induced current's direction (read from the galvanometer deflection) to verify Lenz's law by checking it opposes the magnet's approach/withdrawal.

## Observation Table

| S. No. | Turns N | Magnet speed v (cm/s) | Peak field B_max (mT) | Peak EMF ε (mV) | Direction (approach/withdraw) |
|---|---|---|---|---|---||
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
...
*Induced EMF vs Time (magnet passing through coil) &mdash; bipolar pulse: + then − (or vice versa)*
...
<text x="320.0" y="22" text-anchor="middle" font-size="17" font-weight="700" fill="#0B173A" font-family="Inter,Helvetica,sans-serif">Induced EMF vs Time (magnet passing through coil)</text>
...
<text x="345" y="500" text-anchor="middle" font-size="13" fill="#0B173A" font-family="Inter,Helvetica,sans-serif">time (a.u.) &#8594;</text>
<text x="20" y="252" text-anchor="middle" font-size="13" fill="#0B173A" font-family="Inter,Helvetica,sans-serif" transform="rotate(-90 20 252)">EMF (mV) &#8594;</text>
</svg>
</div>
<div class="apparatus-3d-caption" style="background:var(--lab-navy);"><b>15&times;15 grid</b> &mdash; choose your own scale per square; plot from your observation table. Expected trend: <i>bipolar pulse: + then − (or vice versa)</i></div>
</div>
...
| Case | EMF observed | EMF predicted (−N dΦ/dt) | Lenz's law direction verified? |
|---|---|---|---||
|  |  |  |  |

## Precautions

- Keep the magnet's motion smooth and at a controlled, known speed to get a clean, repeatable EMF pulse.
- Note carefully which face of the magnet (N or S pole) enters the coil first — it fixes the sign of the first EMF lobe.
- Vary only one control (speed, turns, or field strength) at a time to isolate each dependence.
- Remember a stationary magnet inside a stationary coil induces exactly zero EMF, however strong the field.

## Maximum Permissible Error

$$ Δε/ε = ΔN/N + ΔB_max/B_max + Δv/v $$

For a magnet swept through at speed v, peak EMF scales with N, the field's peak value and its rate of change (∝ v), so fractional errors add.

<div class="error-calc" data-exp="faraday-induction"></div>
...
3. Derive the motional EMF formula ε = BLv from Faraday's law for a rod sliding on rails in a magnetic field.
