...
<div class="apparatus-3d" data-setup="gauss-law-flux"></div>

<div class="apparatus-photo">
<img src="../../assets/apparatus/gauss-law-flux.png" alt="Apparatus layout diagram for this experiment">
...
<div class="apparatus-3d-caption"><b>3-D apparatus schematic</b> &middot; drag to rotate, scroll to zoom &middot; A virtual charge enclosed within a draggable/resizable Gaussian surface (sphere, cube or cylinder); the simulator numerically integrates E·dA over the chosen surface and displays the running flux total.</div>
</div>
...
Gauss's law states that the net electric flux Φ_E through any closed surface equals the enclosed charge divided by ε₀: Φ_E = ∮ E·dA = Q_enc/ε₀. This holds regardless of the surface's shape and regardless of where the enclosed charge sits inside it, and charges outside the surface contribute zero net flux. For distributions with sufficient symmetry (spherical, cylindrical, planar), Gauss's law lets the field be obtained directly without integrating Coulomb's law: a spherical shell of charge Q gives E = kQ/r² outside and E = 0 inside; an infinite line charge of linear density λ gives E = λ/(2πε₀ρ); an infinite charged sheet of surface density σ gives a uniform field E = σ/(2ε₀) independent of distance from the sheet.
...
| Symbol | Meaning |
|---|---|
| Φ_E | net electric flux through the closed surface (N·m²/C) |
| Q_enc | total charge enclosed by the surface (C) |
| ε₀ | permittivity of free space |

## Interactive Controls

Launch the [simulator](../../../labs/advanced-electricity-magnetism-lab.html){target=_blank} and use the following controls on the **Gauss's Law — Electric Flux Through Closed Surfaces** panel:

| Control | Symbol | Range | Default |
|---|---|---|---|
| Enclosed charge | Q_enc | -20–20 nC | 10 nC |
| Charge placed outside surface | Q_out | -20–20 nC | 5 nC |
| Gaussian sphere radius | R | 5–40 cm | 15 cm |
| Surface shape | shape | –  | sphere  |

## Procedure

1. Place a known charge Q_enc at the centre of a spherical Gaussian surface of radius R and record the computed flux.
2. Change R (keeping Q_enc fixed) and verify the flux is independent of the radius chosen.
3. Move Q_enc off-centre within the same surface and confirm the total flux is unchanged.
4. Add a second charge Q_out entirely outside the surface and verify it contributes zero net flux, even though it changes the local field at the surface.
5. Switch the Gaussian surface shape from sphere to cube to cylinder (same enclosed charge) and confirm the flux total is identical in every case.
6. Use the flux value to back-calculate E for a spherical shell and compare with the Coulomb's-law field at the same radius.

## Observation Table

| S. No. | Surface shape | Q_enc (nC) | Q_out (nC) | Computed flux Φ (N·m²/C) | Q_enc/ε₀ (theory) |
|---|---|---|---|---|---||
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
...
*Field of a Charged Sphere: E vs r &mdash; E=0 inside, E∝1/r² outside*
...
<text x="320.0" y="22" text-anchor="middle" font-size="17" font-weight="700" fill="#0B173A" font-family="Inter,Helvetica,sans-serif">Field of a Charged Sphere: E vs r</text>
...
<text x="345" y="500" text-anchor="middle" font-size="13" fill="#0B173A" font-family="Inter,Helvetica,sans-serif">r (cm) &#8594;</text>
<text x="20" y="252" text-anchor="middle" font-size="13" fill="#0B173A" font-family="Inter,Helvetica,sans-serif" transform="rotate(-90 20 252)">E (relative units) &#8594;</text>
</svg>
</div>
<div class="apparatus-3d-caption" style="background:var(--lab-navy);"><b>15&times;15 grid</b> &mdash; choose your own scale per square; plot from your observation table. Expected trend: <i>E=0 inside, E∝1/r² outside</i></div>
</div>
...
| Case | Flux measured | Flux predicted (Q_enc/ε₀) | % deviation |
|---|---|---|---||
|  |  |  |  |

## Precautions

- Make sure the chosen surface is genuinely closed (no gaps) before reading off the flux, or the numerical integration will be inaccurate.
- Distinguish carefully between charges enclosed by the surface and charges merely near it but outside.
- For the cylindrical surface, remember to include the flux through the flat end-caps as well as the curved side.
- Keep the enclosed charge exactly at stated positions when comparing symmetric vs asymmetric placements.

## Maximum Permissible Error

$$ ΔΦ/Φ = ΔQ_enc/Q_enc $$

Since Φ = Q_enc/ε₀ with ε₀ an exact constant, the fractional error in flux equals the fractional error in the enclosed charge measurement.

<div class="error-calc" data-exp="gauss-law-flux"></div>
...
6. How does Gauss's law extend to Gauss's law for magnetism (∇·B = 0), and what does that imply physically?
