...
1. Prepends a "Back to Course Portal" banner to the top of every
   documentation page (the simulator and this user guide previously had no
   way to navigate back to the course portal). On each of the 15 experiment
...
def on_page_markdown(markdown, page, config, files):
    ...
    if is_experiment:
        booklet_link = (
            ...
            f'({asset_prefix}assets/lab-record-booklet.pdf){{target=_blank}} '
    ...
    header_md = (
        ...
        f'&larr; [Back to Course Portal]({prefix}index.html) '
        f'&middot; [Launch Simulator]({prefix}labs/advanced-mechanics-matter-lab.html){{target=_blank}}'
        f'{booklet_link}'
        "</div>\n\n"
    )
